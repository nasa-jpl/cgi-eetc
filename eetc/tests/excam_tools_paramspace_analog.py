...
from eetc.excam_tools import calc_gain_fixed_Ntime, _ENF
...
#t_tot = np.logspace(0, 3 ,num=27, base=10);
...
t0 = 200
rn = 100
# putting in some infrastructure to test the fsolve calls.
# they seem fine.  I put in a tolerance of tol, which is 1e-30 by default, but
# that made fsolve worse in some cases (made flag not 1 but 3 or 5) even though
# the output was the expected solution. So I leave tol out of the fsolve calls.
def emrail0(g, cic, t0, fluxe_bright):
        return g*(fluxe_bright*t0 + darke*t0 + cic) + \
            n*_ENF(g, Nem)*g*np.sqrt(fluxe_bright*t0 + darke*t0 +
            cic) - alpha1*fwc_em

def snr_deriv_exp(g, fluxe):
            return 2*(fluxe*t0 + darke*t0 + cic)*g*(1-g+Nem)-g**(1/Nem)*Nem*(
                (fluxe*t0 + darke*t0 + cic)*g+2*rn**2)

def fsolve_emrail0_tester():
    good_flag_1 = 0
    good_flag_other = 0
    bad_flag_1 = 0
    bad_flag_other = 0
    for i in flu_b:
        with warnings.catch_warnings():
            ...
            gsol, _, flag, _ = scipy.optimize.fsolve(func=emrail0, x0=1,
                args=(cic, t0, i), full_output=True)
        if emrail0(gsol[0], cic, t0, i) < 1e-4 and \
            emrail0(gsol[0], cic, t0, i) > -1e-4:
            if flag==1:
                good_flag_1 += 1
            else:
                good_flag_other += 1
            ...
            if flag==1:
                ...
                bad_flag_other += 1
    return good_flag_1, good_flag_other, bad_flag_1, bad_flag_other

good_flag_1, good_flag_other, bad_flag_1, bad_flag_other = \
    fsolve_emrail0_tester()

print("emrail test:")
print('good_flag_1:  ', good_flag_1)
print('good_flag_other:  ', good_flag_other)
print('bad_flag_1:  ', bad_flag_1)
print('bad_flag_other:  ', bad_flag_other)


def fsolve_snr_deriv_exp_tester():
    good_flag_1 = 0
    good_flag_other = 0
    bad_flag_1 = 0
    bad_flag_other = 0
    for i in flu:
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            g_vals, _, flag, _ = scipy.optimize.fsolve(func=snr_deriv_exp,
                            args=(i), x0=1, full_output=True)
        for j in g_vals:
            if snr_deriv_exp(j, i) < 1e-4 and \
                snr_deriv_exp(j, i) > -1e-4:
                if flag==1:
                    good_flag_1 += 1
                else:
                    good_flag_other += 1
...
good_flag_1, good_flag_other, bad_flag_1, bad_flag_other = \
    fsolve_snr_deriv_exp_tester()

print("snr_deriv_exp test:")
print('good_flag_1:  ', good_flag_1)
print('good_flag_other:  ', good_flag_other)
print('bad_flag_1:  ', bad_flag_1)
print('bad_flag_other:  ', bad_flag_other)
...
for xi, x in np.ndenumerate(flu):
    for yi, y in np.ndenumerate(flu_b):
        if y > x:
            ...
            [gain,t,Nf,SNR, ttot] = calc_gain_fixed_Ntime(#target_snr=args.snr,
                            t_tot=200,  #1e-8,  #achievable for any part of parameter space
                            fluxe=x,
                            fluxe_bright = y,
                            darke=8.33e-4,
                            cic=0.02,
            ...
            # XXX check that there aren't too many infeasible solutions, and that ttot is within 1% of t_tot for hard_limit=False
            if ttot == -1:
                opt_minus1 += 1
            else:
                if not np.isclose(ttot, Nf*t, rtol=.1):
                    opt_3[yi,xi] = 1
...
print("number of times opt=-1:  ",np.sum(opt_minus1), '\n')
print("number of times disagreement b/w ttot and N*t:  ",np.sum(opt_3))
