...
def calc_gain_exptime(target_snr, fluxe, fluxe_bright, darke, cic, rn, X, a,
    ...
    if (fluxe_bright*tmin + darke*tmin + cic) + \
        (n*1*_SNR_CR_den(1, Nem, rn, fluxe_bright, tmin,
...
#change from excam_tools.py _________________
        if (res1_cond==False) and (snr1/target_snr < 1-delta_constr) and (target_snr !=100):
            print("rail(np.array([g1,tfr1]))/(alpha0*fwc)/(1+delta_constr): ",
            rail(np.array([g1,tfr1]))/(alpha0*fwc)/(1+delta_constr))
            print("em_rail(np.array([g1,tfr1]))/(alpha1*fwc_em)/(1+delta_constr): ",
            em_rail(np.array([g1,tfr1]))/(alpha1*fwc_em)/(1+delta_constr))
            print("snr1/target_snr/(1-delta_constr): ", snr1/target_snr/(1-delta_constr))
        if (res2_cond==False) and (snr2/target_snr < 1-delta_constr) and (target_snr !=100):
            print("rail(np.array([g2,tfr2]))/(alpha0*fwc)/(1+delta_constr): ",
            rail(np.array([g2,tfr2]))/(alpha0*fwc)/(1+delta_constr))
            print("em_rail(np.array([g2,tfr2]))/(alpha1*fwc_em)/(1+delta_constr): ",
            em_rail(np.array([g2,tfr2]))/(alpha1*fwc_em)/(1+delta_constr))
            print("snr2/target_snr/(1-delta_constr): ", snr2/target_snr/(1-delta_constr))
...
if __name__ == "__main__":
    ...
    out = calc_gain_exptime(target_snr=0.0003703515375711464*(1-1e-6), #args.snr,
                            fluxe=3.359818286283781e-07,    # 0.00737866,  #eetc Issue #42
                            fluxe_bright=1.43844988828766,   #0.0737866,
